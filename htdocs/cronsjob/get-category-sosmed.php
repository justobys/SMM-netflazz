...
require '../config.php';
   ...
   if ( $json_result['status'] == false ) {
      ...
      while ( $i < count($json_result['data']) ) {
         ...
         $cek_kategori = $conn->query("SELECT * FROM kategori_layanan WHERE kode = '$kategori' AND server = 's1'");
         $data_kategori = $cek_kategori->fetch_assoc();
         
         if ( $cek_kategori->num_rows > 0 ) {
            echo "Data Sudah ada di Database<br><br>";
         } else {
            $insert = $conn->query("INSERT INTO kategori_layanan VALUES ('', '$kategori', '$kategori', 'Sosial Media', 's1')");
            if ( $insert == TRUE ) {
               echo "<font color='green'>Success Insert Kategori</font><br> $kategori <br><br>";
            } else {

...
require_once "../config.php";
    ...
    if($action->from == '168' AND preg_match("/Anda menerima Pulsa dari/i", $pesan_isi)) {
         $pesan_isi = $action->message;
         $CheckHistory = $conn->query("SELECT * FROM deposit WHERE tipe = 'Transfer Pulsa' AND provider = 'XL' AND status = 'Pending' AND date = '$date'");
         if (mysqli_num_rows($CheckHistory) == 0) {
             ...
             while($DataDeposit = mysqli_fetch_assoc($CheckHistory)) {
                        ...
                        $cekpesan = preg_match("/Anda menerima Pulsa dari $sender sebesar Rp$jumlah_transfer/i", $pesan_isi);
